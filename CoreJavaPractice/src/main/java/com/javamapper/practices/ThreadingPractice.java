...
public class ThreadingPractice {
	...
	public void singleObjectThreadCommunication() {
		...
		Thread producerThread = new Thread(() -> {
			List<String> dataSet = Arrays.asList("streamBegin","stream1","stream2","stream3","stream4","stream5","stream6","stream7","stream8","streamEnd");
	...
	public void interruptingThread() {
		Thread waitingThread = new Thread(() -> {
			try {
				...
				Thread.currentThread().interrupt();
				/**
				 * Not allow to process child thread then throw
				 * throw new RuntimeException("Blocker waitingThread Thread interrupted."+e);  
				 */
			}
	...
	public void joinExample() {
		Thread th1=new Thread(()->{logger.info("task-1 called");
			try {
				Thread.sleep(10000);
			} catch (InterruptedException e) {
				logger.error(ERROR_LOG, ExceptionUtils.getStackTrace(e));
		...
		th1.start();
		try {
			//Running/Current(main) thread will wait to complete task1 Then processed to next line  
			th1.join();
		} catch (InterruptedException e) {
			logger.error(ERROR_LOG, ExceptionUtils.getStackTrace(e));
			Thread.currentThread().interrupt();
	...
	public void threadPriorityExample() {
		...
		/*
		 * Result
		 * As Priority high for th3 so task-3 start early compare to task-2
	...
	/**1)
	 * New single thread executor example.
	 */
	...
	/***
	 * Executor AND ExecutorService both are interface
	 * Executors is Factory/utility class
	 * Executors use for Executor, ExecutorService, ScheduledExecutorService, ThreadFactory, and Callable classes
	 * 2)
	 */
	public void newFixedThreadPoolLimitedTaskExample() {
		...
		try {
			/**
			 * submit is method of ExecutorService interface 
			 * Take Callable object in argument,In another overload method we pass Runnable with another argument of return value
			 */
			...
			/**
			 * execute is method of Executor interface
			 * Take Runnable object in argument
			 */
			...
			logger.info("task1Return:{} taskWithoutReturn1:{} taskWithoutReturn2:{} taskTake5Sec:{}",task1Return.get(),taskWithoutReturn1Return.get(),taskWithoutReturn2Return.get(),task3Return.get());
		} catch (InterruptedException | ExecutionException e) {
			logger.error(ERROR_LOG,ExceptionUtils.getStackTrace(e));
			Thread.currentThread().interrupt();
		}
	...
	public void newScheduledThreadPoolExample() {
		...
		executorService.scheduleWithFixedDelay(taskRunnable1LogTime, 2L,5L, TimeUnit.SECONDS);//Delay always fixed 5sec     | initial delay 2Sec
		executorService.scheduleAtFixedRate(taskRunnable2LogTime, 2L,10L, TimeUnit.SECONDS);//After each 10Sec task execute | initial delay 2Sec

		ScheduledFuture<Integer> returnGet = executorService.schedule(taskCallable1LogTime, 5, TimeUnit.SECONDS);//schedule task after 5sec
		try { 
	...
	public void newFixedThreadPoolExample() {
		...
		try {
			List<Future<Integer>> futures = executorService.invokeAll(List.of(taskCallable1Sleep1Sec,taskCallable2Sleep1Sec,taskCallable1Sleep3Sec,taskCallable4Sleep1Sec,taskCallable5Sleep1Sec));
	...
	public void newFixedThreadPoolTimeOutExample() {
		...
		ExecutorService executorService = Executors.newFixedThreadPool(3);
		try {
			List<Future<Integer>> futures = executorService.invokeAll(List.of(taskCallable1Sleep1Sec,taskCallable2Sleep1Sec,taskCallable1Sleep3Sec),2,TimeUnit.SECONDS);
			futures.stream().map(future->{
				try {
	...
	public void threadPoolExecutorExample() {
		/**
		 * ThreadPoolExecutor class that implements both interfaces(Executor, ExecutorService).
		 */
		ExecutorService executorService = new ThreadPoolExecutor(10, 50, 5L, TimeUnit.MILLISECONDS,new LinkedBlockingQueue<>());
		try {
			List<Future<Integer>> futures = executorService.invokeAll(List.of(taskCallable1Sleep1Sec,taskCallable2Sleep1Sec,taskCallable1Sleep3Sec),2,TimeUnit.SECONDS);
	...
	public void threadTooManyJob() {
		ThreadPoolExecutor executorService = new ThreadPoolExecutor(10, 50, 5L, TimeUnit.HOURS,new LinkedBlockingQueue<>());
	...
	public void threadPoolExecutorWithLimitedQueueSizeExample() {
		ThreadPoolExecutor executorService = new ThreadPoolExecutor(3,3,1,TimeUnit.HOURS,new ArrayBlockingQueue<>(10, false));
	 ...
	 * semaphore allow access to shared resource : permits are greater than zero.
	 * semaphore does not allow access to shared resource : permits are zero or less than zero.
	 *
	 *semaphore(int permits) // initial number, If it negative it means first semaphore.releases must occur before any semaphore.acquires
	 *semaphore(int permits,boolean fair) //fair : True -> when waiting threads are granted a permit in the order in which they requested access
	 *
	...
	/***
	 * Lock use not Synchronized Block
	 * Lock interface with methods(lock,lockInterruptibly,tryLock,unlock) -> ReentrantLock class
	 * ReadWriteLock  with methods( readLock, writeLock) -> ReentrantReadWriteLock class
	 */
	public void lockExample() {
        ...
        Stream.iterate(101, n->n+1).limit(20).forEach(n->list.add(Pair.of(n, executorService.submit(new Booking(new Room(),n),"COMPLETE"))));
	...
	public void readWriteLockExample() {
		...
		/***
		 *  ReadLock  :  If no threads are writing then -> any number of threads acquire for read lock
		 *  WriteLock :  If no threads are reading and writing then -> a thread acquire for write lock
		 */
        ...
        Stream.iterate(0, n->n+1)
        	  .limit(limitVal)
        	  .forEach(n->list.add(Pair.of(String.format(WRITE_INT,n), executorService.submit(()->fileAction.write(String.format("I am not fine %d",n)),"COMPLETE"))));
        
        list.add(Pair.of(String.format(REMOVE_INT, limitVal),executorService.submit(fileAction::remove,"COMPLETE")));
       
        list.add(Pair.of(String.format(WRITE_INT, limitVal+1),executorService.submit(()->fileAction.write(String.format("I am fine %d", limitVal+1)),"COMPLETE")));
	...
	public void stampedLockExample() {
        ...
        Future<Optional<Boolean>> containsOffWhenWriterOn = executorService.submit(()->fileAction.containsOffWhenWriterOn("I am not fine 1"));        
        list.add(Pair.of(String.format(REMOVE_INT, limitVal),executorService.submit(fileAction::remove,"COMPLETE")));
        ...
        /***
         * In real time we can think above call and there return trigger from different services.. So result will not goes sequential like below
         */
        list.stream().forEach(n-> {
			try {
				logger.info("{} Value {}",n.getKey(),n.getValue().get());
