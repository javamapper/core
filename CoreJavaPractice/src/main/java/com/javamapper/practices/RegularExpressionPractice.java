...
public class RegularExpressionPractice {
	...
	public void regExGrouping() {
		...
		System.out.println(strr.replaceAll("(?<charGrp>[a-z]*)(?<digitGrp>[0-9.,]*)","${digitGrp}${charGrp}"));// we can use "$2$1" in this expression
		...
		System.out.println(strr.replaceAll("([a-z]*)([0-9.,]*)","$2$1"));
		...
		System.out.println(strr.replaceAll("([^\\d\\W]{3})(\\d{2})(\\s+)","$2$1$3")); 
		...
		System.out.println(strr.replaceAll("([\\w&&[^\\d_]]{3})(\\d{2})(\\s+)","$2$1$3")); 
		//
		System.out.println("Hello, Anil".replaceAll("(\\w)([\\.,])(\\s+)", "$1$3"));
		// Remove whitespace between word and , just like trim spaces which is between word and ,
		System.out.println("hello  ,".replaceAll("(\\w)(\\s+)([\\,])", "$1$3"));
		
		// A String start with number then push it back/last
		System.out.println("9003Anil hello654 98Rana goo5442aa".replaceAll("(^|\\s+)([0-9]+)([a-zA-Z]+)(\\s+)","$1$3$2$4"));
		
		System.out.println("tewtw@gm".replaceAll("([a-zA-Z]{7})(\\@)(\\w+)","$1")); 
